...
class UsaMap extends Component {
  ...
  updateStateName = selection => {
    let state = selection.target.id;
    let fullStateName = '';

    if (state !== '' && state !== 'outlines') {
      try {
        fullStateName = statesData[state][0].full_state_name;
      } catch (err) {
        console.log(err);
      }
    }

    this.setState({ stateName: fullStateName });
  }

  selectState = (e) => {
    const BLUE = '#13a4d2';
    let selectedState = e.target.id;
    let senators;
    let senator1;
    let senator2;
    let senator1Link;
    ...
    if (selectedState !== '' && selectedState !== 'outlines') {
      senators = statesData[selectedState];
      senator1 = `${senators[0]["first_name"]} ${senators[0]["last_name"]}`;
      senator1Link = senators[0]["wikipedia"];
      senator2 = `${senators[1]["first_name"]} ${senators[1]["last_name"]}`;
      senator2Link = senators[1]["wikipedia"];
    }

    if (senator1 && senator2 && this.state.stateName && senator1Link && senator2Link) {
      this.setState({
        sen1: senator1,
        sen2: senator2,
    ...
    if (e.target.style.fill === BLUE) {
      ...
      let pathList = document.getElementsByClassName('map')[0].getElementsByTagName('path');
      for (let i = 0; i < pathList.length; i++) {
        pathList[i].style.fill = '';
      }
  ...
  render() {
    return (
      <div>
        <div className="state_name_div">
          <h1>{this.state.stateName}</h1>
        </div>

        <svg
          className="map"
          onMouseOver={this.updateStateName}
